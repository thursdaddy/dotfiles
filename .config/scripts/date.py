...
def thedate(config):
    if config['format']  == "date":
        ...
        result = d.now().strftime(" %-I:%M:%S %p  |")
        return result

# set config path
home_dir = os.getenv("HOME")
config_path = ("%s/.config/py_date/py_date.conf" % home_dir)
if not os.path.exists("%s/.config/py_date" % home_dir):
    os.makedirs("%s/.config/py_date" % home_dir)
...
if not os.path.exists("%s/.config/py_date/py_date.conf" % home_dir):
